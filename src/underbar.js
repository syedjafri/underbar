...
(function() {
 ...
 window._ = {};
  ...
  _.each = function(collection, iterator) {
      // check if its an Object, loop over keys (in calling object) print values.
  ...
  _.uniq = function(array) {
    ...
    // Approach: Make key-value pair. 
    // Keys will be unique numbers, value will be number of occurance 
  ...
  _.map = function(collection, iterator) {
    ...
    if (Object.prototype.toString.call(collection)=="[object Object]"){
      for (var key in collection){
          if (collection.hasOwnProperty(key)){
            mappedObj[key] = iterator(collection[key], key, collection);
          }
      }
      return mappedObj;
  ...
  _.every = function(collection, iterator) {
    ...
    var iteratorPresent = arguments.length==2?true:false;
    ...
    return _.reduce(result, function(falsy, val){
      return (falsy && val);
    }, true);
  ...
  _.some = function(collection, iterator) {
    ...
    var result = _.every(collection, function (val){
      if (!iteratorPresent){
        return (!val);
      } else {
        return !iterator(val);
  ...
  _.memoize = function(func) {
    ...
    return function() {
      ...
      if (alreadyCalled.hasOwnProperty(args)){
  ...
  _.shuffle = function(array) {
    ...
    var shuffled = [];
    var max = arrayCopy.length;
    var index;

    while(shuffled.length<max){
      index=Math.floor(Math.random()*max);
      if (arrayCopy[index]!==undefined){
...
/* Would love to continue once I make the deadlines for precourse work :) */
